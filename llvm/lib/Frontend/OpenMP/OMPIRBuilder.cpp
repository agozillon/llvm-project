...
static Function *createOutlinedFunction(
  ...
  for (auto InArg : zip(Inputs, ArgRange)) {
    ...
    // but are technically seperate arguments to the kernel for all intents
    // and purposes. If we have mapped a segment that requires a GEP into the
    // 0-th index, it will fold into an referal to the Global, if we then
    // encounter this folded GEP during replacement all of the references to
    // the Global in the kernel will be replaced with the argument we have
    // generated that corresponds to it, including any other GEP's that refer
    // to the Global that may be other arguments. This will invalidate all of
    // the other preceding mapped arguments that refer to the same global that
    // may be seperate segments. To prevent this, we defer global processing
    // until all other processing has been performed.
