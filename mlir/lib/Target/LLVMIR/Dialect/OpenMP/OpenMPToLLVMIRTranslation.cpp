...
static void sortMapIndices(llvm::SmallVector<size_t> &indices,
  ...
  if (indexAttr.empty() || indexAttr.size() == 1 || indices.empty() || indices.size() == 1)
    return;
...
static void getOverlappedMembers(llvm::SmallVector<size_t> &overlapMapDataIdxs, MapInfoData &mapData, omp::MapInfoOp parentOp) {
  ...
  for (auto [memIndex, indicesAttr] : llvm::enumerate(indexAttr))
    memberByIndex.push_back(std::make_pair(memIndex, mlir::cast<mlir::ArrayAttr>(indicesAttr)));
  ...
  // Remove elements from the vector if there is a parent element that supersedes
  // it. i.e. if member [0] is mapped, we can remove members [0,1], [0,2].. etc.
...
static llvm::omp::OpenMPOffloadMappingFlags mapParentWithMembers(
  ...
  if (!parentClause.getPartialMap()) {
    ...
    if (targetDirective != TargetDirective::Target &&
        isDeclareTargetTo(parentClause.getVarPtr()
...
static void processMapWithMembersOf(
  ...
  llvm::omp::OpenMPOffloadMappingFlags memberOfParentFlag =
      mapParentWithMembers(moduleTranslation, builder, ompBuilder, dl,
                           combinedInfo, mapData, mapDataIndex, targetDirective);
...
convertOmpTargetData(Operation *op, llvm::IRBuilderBase &builder,
  ...
  auto genMapInfoCB =
    ...
    builder.restoreIP(codeGenIP);
    genMapInfos(builder, moduleTranslation, DL, combinedInfo, mapData, targetDirective);
    return combinedInfo;
  };
